...
export function BookingConflictViews({ api, referenceData, token }: { api: OpsApi; referenceData: BookingReferenceData; token: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await api.listBookingConflicts(token, { page, perPage: 20, status })
...
function ConflictTable({ conflicts, onResolve, referenceData }: { conflicts: BookingConflict[]; onResolve: (item: BookingConflict) => void; referenceData: BookingReferenceData }) {
  return <DataTable><thead><tr className="border-b border-[var(--border)] bg-[var(--muted)] text-left text-xs text-[var(--muted-foreground)]"><th className="px-4 py-3">冲突类型</th><th className="px-4 py-3">教师 / 学员</th><th className="px-4 py-3">时间</th><th className="px-4 py-3">关联记录</th><th className="px-4 py-3">状态</th><th className="px-4 py-3 text-right">操作</th></tr></thead><tbody>{conflicts.map((item) => <tr className="border-b border-[var(--border)]" key={item.id}><td className="px-4 py-3"><p className="font-semibold">{conflictTypeLabel(item.type)}</p><p className="text-xs text-[var(--muted-foreground)]">{conflictDescription(item)}</p></td><td className="px-4 py-3">{teacherName(item.teacherId, referenceData)}{item.studentId ? <p className="text-xs text-[var(--muted-foreground)]">学员 {item.studentId}</p> : null}</td><td className="px-4 py-3">{formatDateTime(item.startAt)}<p className="text-xs text-[var(--muted-foreground)]">{formatTimeRange(item.startAt, item.endAt)}</p></td><td className="px-4 py-3 text-xs text-[var(--muted-foreground)]">{item.sessionId ? `课堂 ${item.sessionId}` : item.appointmentId ? `预约 ${item.appointmentId}` : '无'}</td><td className="px-4 py-3"><Badge tone={item.status === 'open' ? 'amber' : 'green'}>{item.status === 'open' ? '待处理' : '已处理'}</Badge></td><td className="px-4 py-3 text-right">{item.status === 'open' ? <Button onClick={() => onResolve(item)} type="button" variant="secondary">记录处理结果</Button> : <span className="text-xs text-[var(--muted-foreground)]">{item.resolution || '已关闭'}</span>}</td></tr>)}</tbody></DataTable>
}

function ConflictCard({ conflict, onResolve, referenceData }: { conflict: BookingConflict; onResolve: () => void; referenceData: BookingReferenceData }) {
  return <article className="grid gap-3 px-4 py-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{conflictTypeLabel(conflict.type)}</h3><p className="text-xs text-[var(--muted-foreground)]">{teacherName(conflict.teacherId, referenceData)} · {formatDateTime(conflict.startAt)}</p></div><Badge tone={conflict.status === 'open' ? 'amber' : 'green'}>{conflict.status === 'open' ? '待处理' : '已处理'}</Badge></div><p className="text-sm text-[var(--muted-foreground)]">{conflictDescription(conflict)}</p>{conflict.status === 'open' ? <Button onClick={onResolve} type="button" variant="secondary">记录处理结果</Button> : null}</article>
}

function ResolveConflictDialog({ conflict, loading, onClose, onResolve }: { conflict: BookingConflict; loading: boolean; onClose: () => void; onResolve: (resolution: string) => Promise<void> }) {
  const [resolution, setResolution] = useState('已核对排课并完成线下调整')
  return <DialogShell description="该操作只关闭冲突记录，不会自动移动或删除历史课堂。" onRequestClose={onClose} size="compact" title="记录冲突处理结果"><form className="grid gap-4 p-5" onSubmit={(event) => { event.preventDefault(); if (resolution.trim().length >= 4) void onResolve(resolution.trim()) }}><div className="rounded-md border border-[var(--border)] bg-[var(--muted)] p-3 text-sm"><p className="font-semibold">{conflictTypeLabel(conflict.type)}</p><p className="mt-1 text-[var(--muted-foreground)]">{formatDateTime(conflict.startAt)} {formatTimeRange(conflict.startAt, conflict.endAt)}</p></div><Field htmlFor="conflict-resolution" label="处理结果"><Textarea id="conflict-resolution" minLength={4} onChange={(event) => setResolution(event.target.value)} required value={resolution} /></Field><div className="flex justify-end gap-2"><Button onClick={onClose} type="button" variant="secondary">取消</Button><Button disabled={loading || resolution.trim().length < 4} type="submit">确认关闭冲突</Button></div></form></DialogShell>
}
...
function teacherName(id: string | undefined, refs: BookingReferenceData) { return refs.teachers.find((item) => item.id === id)?.name || (id ? `教师 ${id}` : '教师待确认') }
