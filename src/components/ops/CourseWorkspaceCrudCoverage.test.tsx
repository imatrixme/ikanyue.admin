...
describe('course CRUD workspaces', () => {
  it('covers generic resource details, mobile actions, editing, creation, and status changes', async () => {
    ...
    expect(await screen.findByRole('table')).toHaveTextContent('请在技术信息中查看')
    expect(screen.getByRole('table')).not.toHaveTextContent('{"level":1}')
    expect(screen.getAllByText('仅标题').length).toBeGreaterThan(0)
    expect(screen.getAllByText('OP-001').length).toBeGreaterThan(0)
    expect(screen.getAllByText(/未找到业务记录/).length).toBeGreaterThan(0)
  ...
  it('covers class create, edit, membership, transfer, teacher, and mobile actions', async () => {
    ...
    await chooseReference(user, drawer, '选择要加入的学员', '李同学')
    ...
    await user.click(within(drawer).getByRole('button', { name: /移除李同学/ }))
    await waitFor(() => expect(membership).toHaveBeenCalledWith('token', 'class_1', 'student_2', 'cancelled'))
    await chooseReference(user, drawer, '选择授课教师', '周老师')
    await user.selectOptions(within(drawer).getByLabelText('教学角色'), 'assistant')
  ...
  it('covers class load, detail, save, and sparse relation fallbacks', async () => {
    ...
    expect(drawer).toHaveTextContent('CODE-ONLY')
    expect(drawer).toHaveTextContent('EMPTY-TIME')
    expect(drawer).toHaveTextContent('时间待定')
    await user.click(within(drawer).getByRole('button', { name: '关闭弹窗' }))
...
async function fillClass(user: ReturnType<typeof userEvent.setup>, dialog: HTMLElement, name: string, code: string) {
  ...
  await chooseReference(user, dialog, '开设课程', '一对一声乐')
  await chooseReference(user, dialog, '默认扣减课时', '综合声乐')
  await user.type(within(dialog).getByLabelText('学期开始'), '2026-09-01')
  await user.type(within(dialog).getByLabelText('学期结束'), '2027-02-28')
  await user.clear(within(dialog).getByLabelText('班级人数上限'))
  await user.type(within(dialog).getByLabelText('班级人数上限'), '10')
  await user.type(within(dialog).getByLabelText('默认上课地点'), '二号教室')
}
