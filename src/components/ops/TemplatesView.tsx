    ...
    <Panel>
      <SectionHeader>
        <div>
          ...
          <p className="text-sm text-[var(--muted-foreground)]">模板结构、评分规则和报告配置统一版本化。</p>
        </div>
        <Button onClick={onCreate} icon={<Plus className="h-4 w-4" aria-hidden="true" />}>新建模板</Button>
      </SectionHeader>
      <div className="grid gap-3 p-4">
        {(data?.items || []).map((template) => (
          <article key={template.id} className="rounded-md border border-[var(--border)] bg-[var(--card)] p-4 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-[var(--muted-foreground)]">v{template.version} · {template.schemaJson.sections.length} 个维度 · {template.scoringJson.type}</p>
              </div>
            ...
            <div className="mt-4 grid gap-2 md:grid-cols-3">
              {template.schemaJson.sections.map((section) => (
                <div key={section.key} className="rounded-md border border-[var(--border)] bg-[var(--muted)]/35 p-3">
                  <div className="font-semibold">{section.title}</div>
                  <div className="mt-1 text-sm text-[var(--muted-foreground)]">权重 {Math.round(section.weight * 100)}% · {section.items.length} 项</div>
                </div>
              ))}
            </div>
          </article>
