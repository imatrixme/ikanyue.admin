  ...
  return (
    <div className="grid gap-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs font-medium text-[var(--muted-foreground)]">Dashboard</p>
          <h2 className="mt-1 text-2xl font-semibold tracking-tight">运营总览</h2>
        </div>
      ...
      <Panel>
        <SectionHeader>
          <div>
            ...
            <p className="text-sm text-[var(--muted-foreground)]">审核、发布、报告分享等运营任务集中展示。</p>
          </div>
        </SectionHeader>
        <div className="grid gap-3 p-4 md:grid-cols-3">
          {(data?.pending || []).map((item) => (
            <div key={item.key} className="rounded-md border border-[var(--border)] bg-[var(--muted)]/30 p-4">
          ...
          <div className="rounded-md border border-[var(--border)] bg-[var(--muted)]/30 p-4">
            <div className="text-sm text-[var(--muted-foreground)]">近期报告</div>
            <div className="mt-2 text-2xl font-semibold tabular-nums">7</div>
          </div>
          <div className="rounded-md border border-[var(--border)] bg-[var(--muted)]/30 p-4">
            <div className="text-sm text-[var(--muted-foreground)]">可分享报告</div>
