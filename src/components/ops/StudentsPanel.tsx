...
interface StudentsPanelProps {
  ...
  onOpenOperations?: (student: StudentRecord) => void
...
export function StudentsPanel({ canManage = true, errorMessage, loading, onOpenOperations, onReload, onSave, students }: StudentsPanelProps) {
  ...
  return (
    <WorkspacePanel>
      ...
      <AsyncState empty={paged.items.length === 0 ? <ListEmptyState filtered={activeCount > 0} noun="学员" onCreate={canManage ? () => setEditor(null) : undefined} onReset={resetFilters} /> : undefined} error={students.length === 0 ? errorMessage : undefined} errorTitle="学员列表加载失败" loading={loading && students.length === 0} loadingLabel="正在加载学员..." onRetry={() => void onReload()}><ResponsiveDataRegion desktop={<StudentTable onEdit={canManage ? setEditor : undefined} onOpenOperations={onOpenOperations} students={paged.items} />} mobile={paged.items.map((student) => <StudentCard key={student.id} onEdit={canManage ? () => setEditor(student) : undefined} onOpenOperations={onOpenOperations ? () => onOpenOperations(student) : undefined} student={student} />)} /></AsyncState>
