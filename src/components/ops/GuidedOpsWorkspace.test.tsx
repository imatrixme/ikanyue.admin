...
describe('guided ops workspace', () => {
  it('opens a scenario-first guided dialog and preserves answers across steps', async () => {
    ...
    expect(within(dialog).getByText(/将保存/)).toBeInTheDocument()
    expect(within(dialog).getAllByText('查看详情').length).toBeGreaterThan(0)
  ...
  it('saves, resumes, and deletes guided workflow drafts', async () => {
    ...
    await user.click(screen.getByRole('tab', { name: '创建班级/课包' }))
  ...
  it('resets answers when reopening a different workflow and handles empty relation options', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /发布可报名活动/ }))
    let activityDialog = await screen.findByRole('dialog', { name: '发布可报名活动' })
    ...
    await user.click(within(activityDialog).getByRole('button', { name: '编辑活动简介' }))
    const editor = await screen.findByRole('dialog', { name: '编辑活动简介' })
    await user.click(within(editor).getByRole('tab', { name: '主 Markdown 模式' }))
    await user.type(within(editor).getByLabelText('活动简介 Markdown编辑'), '只测试关闭分支')
    await user.click(within(editor).getByRole('button', { name: '保存内容' }))
    activityDialog = await screen.findByRole('dialog', { name: '发布可报名活动' })
    await user.click(within(activityDialog).getByRole('button', { name: /人物.*谁负责和授课/ }))
    expect(within(activityDialog).getByText('暂无可选项')).toBeInTheDocument()
