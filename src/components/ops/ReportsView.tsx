    ...
    <Panel>
      <SectionHeader>
        <div>
          ...
          <p className="text-sm text-[var(--muted-foreground)]">已提交评估会生成不可变报告快照，再单独创建分享链接。</p>
        </div>
      </SectionHeader>
      <div className="grid gap-3 p-4">
        {(data?.items || []).map((report) => (
          <article key={report.id} className="grid gap-3 rounded-md border border-[var(--border)] bg-[var(--card)] p-4 shadow-sm transition-colors hover:bg-[var(--muted)]/25 md:grid-cols-[1fr_auto] md:items-center">
            <div>
              ...
              <p className="mt-1 text-sm text-[var(--muted-foreground)]">学员 {report.studentId} · 教师 {report.teacherId} · 模板 {report.templateId}</p>
            </div>
      ...
      {shareLink ? (
        <div className="border-t border-[var(--border)] p-4">
          <div className="rounded-md border border-[var(--border)] bg-[var(--muted)]/35 p-3 text-sm">
            <p className="font-semibold">分享链接</p>
            <p className="mt-1 text-[var(--muted-foreground)]">Token: {shareLink.token || '已撤销'}</p>
      ...
      {detail ? (
        <div className="border-t border-[var(--border)] p-4">
          <div className="grid gap-4 rounded-md border border-[var(--border)] bg-[var(--muted)]/25 p-4">
            <div>
              <h3 className="text-lg font-semibold">报告详情</h3>
              <p className="text-sm text-[var(--muted-foreground)]">{detail.title} · {detail.generatedAt}</p>
            </div>
            ...
            {detail.summary ? <p className="text-sm leading-6 text-[var(--foreground)]">{detail.summary}</p> : null}
            <div className="grid gap-2">
              {(detail.sections || detail.score.sections || []).map((section) => (
                <div key={section.key} className="rounded-md border border-[var(--border)] bg-[var(--card)] p-3">
                  <p className="font-semibold">{section.title} {section.score !== undefined ? `· ${section.score}` : ''}</p>
                  {section.comment ? <p className="mt-1 text-sm text-[var(--muted-foreground)]">{section.comment}</p> : null}
                </div>
              ))}
            </div>
