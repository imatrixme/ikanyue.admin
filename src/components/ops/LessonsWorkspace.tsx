...
function SettlementPreviewDialog({ onCancel, onConfirm, preview }: { onCancel: () => void; onConfirm: () => void; preview: SettlementPreview }) { return <DialogShell description="仅预览，不会写入数据。" onRequestClose={onCancel} title="课堂核销预览"><div className="grid gap-5 p-5"><SummaryBand><SummaryMetric label="学员变动" value={preview.studentMovements?.length || 0} /><SummaryMetric label="教师工作量" value={preview.teacherMovements?.length || 0} /><SummaryMetric label="异常" value={preview.exceptions?.length || 0} /></SummaryBand><div className="flex justify-end gap-2"><Button onClick={onCancel} type="button" variant="secondary">返回检查</Button><Button onClick={onConfirm} type="button">进入确认</Button></div></div></DialogShell> }
