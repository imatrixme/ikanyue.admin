...
describe('ops api clients', () => {
  ...
  it('http api builds query URLs and falls back to default error messages', async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce({ ok: true, json: async () => ({ code: 10000, data: { items: [], pagination: { totalItems: 0 } } }) })
      .mockResolvedValueOnce({ ok: true, json: async () => ({ code: 10000, data: { items: [], pagination: { totalItems: 0 } } }) })
      .mockResolvedValueOnce({ ok: true, json: async () => ({ code: 10000, data: { id: 'report_event_1', title: '报告任务' } }) })
      .mockResolvedValueOnce({ ok: true, json: async () => ({ code: 10000, data: { id: 'report_event_1', title: '报告任务更新' } }) })
      .mockResolvedValueOnce({ ok: true, json: async () => ({ code: 10000, data: { items: [], pagination: { totalItems: 0 } } }) })
    ...
    await api.createResource('reportEvents', 'token', { title: '报告任务' })
